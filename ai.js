...
const app = express();
app.use(express.json({ limit: '10mb' })); // Réduit de 50mb à 10mb pour sécurité
app.use(cors());

// 1. On récupère les variables depuis l'environnement Netlify
const MISTRAL_API_KEY = process.env.MISTRAL_API_KEY;
const AGENT_ID = process.env.MISTRAL_AGENT_ID;

// 2. Vérification au démarrage (Utile pour le debug)
if (!MISTRAL_API_KEY) {
  console.warn('⚠️ MISTRAL_API_KEY non définie dans les paramètres Netlify !');
}
if (!AGENT_ID) {
  console.warn('⚠️ MISTRAL_AGENT_ID non défini dans les paramètres Netlify !');
}

// Ensuite, le reste de ton code (app.use, app.post, etc.)
